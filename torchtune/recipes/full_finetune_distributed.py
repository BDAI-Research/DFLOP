...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        print(f"Number of heads | model : {self._model.num_heads}, attention: {self._model.layers[0].attn.num_heads}")
    ...
    def _setup_model(
        ...
        print(f"[0] Number of heads | model : {model.num_heads}, attention: {model.layers[0].attn.num_heads}")
        ...
        if self.parallel_dims.tp_enabled:
            ...
            print(f"[1] Number of heads | model : {model.num_heads}, attention: {model.layers[0].attn.num_heads}")
            if self.tp_plan is not None:
                self.tp_plan = config.instantiate(
                    self.tp_plan,
                    model=model,
                )
                print(f"[2] Number of heads | model : {model.num_heads}, attention: {model.layers[0].attn.num_heads}")
            parallelize_module(
                model,
                self.world_mesh["tp"],
                parallelize_plan=self.tp_plan,
            )
            print(f"[3] Number of heads | model : {model.num_heads}, attention: {model.layers[0].attn.num_heads}")
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    print(f"Number of heads | model : {recipe._model.num_heads}, attention: {recipe._model.layers[0].attn.num_heads}")
    recipe.train()
    recipe.cleanup()
