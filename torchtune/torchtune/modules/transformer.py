...
class TransformerSelfAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        # print(f"[Rank : {torch.distributed.get_rank()}] x type : {x.dtype}, input_pos type : {input_pos.dtype if input_pos is not None else None}")
        h = self.sa_norm(x)
        ...
        # print(f"Attention input shape : {h.shape}")
        # print(f"[Rank : {torch.distributed.get_rank()}] h type : {h.dtype}, mask type : {mask.dtype if mask is not None else None}")
        ...
        # print(f"[Rank : {torch.distributed.get_rank()}] MLP output shape : {mlp_out.shape}")
