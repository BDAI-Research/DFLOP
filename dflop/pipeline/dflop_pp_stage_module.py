...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        v_pp_last_group_ranks : Optional[List[int]] = None,
        llm_first_group_ranks: Optional[List[int]] = None,
        v_tp_first_group_ranks: Optional[List[int]] = None,
        l_tp_first_group_ranks: Optional[List[int]] = None,
        l_tp_size: Optional[int] = None,
        ...
        Args:
            ...
            group (Optional[dist.ProcessGroup]): The process group to use for communication in each module.
        ...
        self.llm_tp_first_group_ranks = l_tp_first_group_ranks
        self.vision_last_group = dist.new_group(v_pp_last_group_ranks)
        self.llm_first_group = dist.new_group(llm_first_group_ranks)
        self.vision_tp_first_group = dist.new_group(v_tp_first_group_ranks)
    ...
    @property
    def is_vision_connect_rank(self):
        if (self.is_last) & (self.is_vision) & (dist.get_rank(self.vision_last_group) == 0):
            return True
        else:
            return False
    @property
    def is_llm_connect_rank(self):
        if (self.is_llm) & (dist.get_rank(self.llm_first_group) == 0):
            return True
        else:
            return False

    @property
    def is_vision_first_tp_ranks(self):
        if (self.is_vision) & (dist.get_rank() in self.vision_tp_first_group_ranks):
            return True
        else:
            return False
    @property
    def is_llm_first_tp_ranks(self):
        if (self.is_llm) & (dist.get_rank() in self.llm_tp_first_group_ranks):
            return True
        else:
            return False
        ...
        for info in recv_infos:
            ...
            if is_fwd: # Forward : (LLM first ranks get activation from vision_connect_rank, rest of ranks get from neighbor ranks)
                ...
                if self.is_llm and self.is_first:
                    peer_global_rank = self.vision_connect_rank
                    ops.append(
                ...
                if self.is_vision_connect_rank:
                    for inf in info:
                        inf.buffer = inf.buffer.to(self.device).detach().requires_grad_(True)
                        peer_global_rank = inf.source
                        if peer_global_rank in self.llm_tp_first_group_ranks:
                            ops.append(
                                dist.P2POp(dist.irecv, inf.buffer, peer_global_rank, None)
                            )
                else:
                    info.buffer = info.buffer.to(self.device).detach().requires_grad_(True)
                    peer_rank = self.stage_index_to_group_rank[info.source]
    ...
    def get_fwd_recv_ops(self, fwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        return self._get_recv_ops(recv_infos, True)
    ...
    def get_bwd_recv_ops(self, bwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        for this stage.
        """
        if not self.has_backward or (self.is_last and self.is_llm):
            return []

        recv_infos = self.grad_recv_info[bwd_chunk_id]
        return self._get_recv_ops(recv_infos, False)
    ...
    def get_fwd_send_ops(self, fwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        if isinstance(output, tuple):
            for out in output:
                attach_output.append(out.to(self.device).requires_grad_(True))
            output = tuple(attach_output)
        else:
            output = output.to(self.device).requires_grad_(True)

        # Unify output form to tuple for easy correspondance with
        # `act_send_info`
        output_tuple = output if type(output) is tuple else (output,)
        # Vision last ranks need to perform dist.all_gather on the output
        if self.is_vision_first_tp_ranks:
            output_list = []
            vision_group_size = self.vision_tp_first_group.size()
            num_images = sum(self.split_sizes[fwd_chunk_id])
            # Split num_images into vision_last_group_size parts, distribute remainder to the first groups
            base = num_images // vision_group_size
            rem = num_images % vision_group_size
        ...
        for idx, out in enumerate(output_tuple):
            dst_stages = self.act_send_info[idx] # Last ranks except vision connect rank will be empty
            dst_idx = 0
            tp_idx = 0
            for dst in dst_stages:
                if dst is None:
                    continue
                if self.is_vision_connect_rank:
                    data_idx = self.data_stage_idx[fwd_chunk_id][dst_idx]
                    indexed_out = [out[i] for i in data_idx]
                    indexed_out = torch.concat(indexed_out, dim=0)                    
                    peer_global_rank = dst
                    logger.debug(
                    ...
                    ops.append(dist.P2POp(dist.isend, indexed_out, peer_global_rank, None))
                    tp_idx += 1
                    if tp_idx == self.l_tp_size:
                        tp_idx = 0
                        dst_idx += 1
                else:
    ...
    def get_bwd_send_ops(self, bwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        for grad, grad_recv_stage in zip(grads_input, self.grad_send_info):
            if isinstance(grad, torch.Tensor) and grad_recv_stage is not None:
                ...
                if self.is_first and self.is_llm:
                    if self.is_llm_first_tp_ranks:
                        peer_global_rank = self.vision_connect_rank
    ...
    def clear_runtime_states(self) -> None:
        ...
        # Caching chunk outputs for final output merge or reduction
        self.data_stage_idx.clear()
        self.data_stage_idx.clear()
        self.split_sizes.clear()
        self.args_recv_info.clear()
        self.act_send_info.clear()
        ...
        if self.is_vision and self.is_last:
            ...
            if self.is_vision_connect_rank:
                if "data_idx" not in kwargs:
                    raise RuntimeError(
                        f"{self.log_prefix} forward_one_chunk requires 'data_idx' in kwargs"
                    )
    ...
    def backward_one_chunk(
        ...
        if self.is_last:
            ...
            # next stage
                ...
                if self.is_vision_connect_rank:
                    ...
                    base = num_images // vision_group_size
                    rem = num_images % vision_group_size
                    grad_num_list = [base + (1 if i < rem else 0) for i in range(vision_group_size)]
                    grad_split_list = list(torch.split(concat_grads_output, grad_num_list, dim=0))
                    futures = []
                    v_tp_size = self.vision_last_group.size() // self.vision_tp_first_group.size()
                    dst_idx = 0
        ...
        if self.is_last and not self.is_first and self.is_llm:
...
class PipelineStage(_PipelineStageBase):
    ...
    def __init__(
        ...
        super().__init__(submodule, stage_index, num_stages, device, group, v_pp_last_group_ranks, llm_first_group_ranks, v_tp_first_group_ranks, l_tp_first_group_ranks, l_tp_size, is_vision, is_llm, dw_builder)
    ...
    def _create_grad_recv_info(
        ...
        if not (self.is_last and self.is_llm):
