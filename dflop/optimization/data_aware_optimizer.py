...
def calculate_peak_mem(model_cur_mem, model_state_mem, act_mem, buffer_ratio=1.2):
...
def precompute_configs(total_gpus, model_mem_factors, llm_act_mem_factors, num_layers, L_MAX_SEQ_LEN, GPU_MEMORY, is_vision, tp_choices):
    """
    각 GPU 수(g)에 대해 가능한 모든 (tp, pp, dp) 조합을 미리 계산합니다.
    종속적인 메모리 제약은 나중으로 미루고, 기본적인 정보만 저장합니다.
    """
    ...
    for g in range(1, total_gpus + 1):
        for tp in tp_choices:
            if g % tp == 0:
                rem_g = g // tp
                for pp in range(1, rem_g + 1):
                    if rem_g % pp == 0:
                        dp = rem_g // pp
                        
                        # 모델 상태(가중치) 메모리만 먼저 계산
                        model_cur_mem, model_state_mem = calculate_model_peak(model_mem_factors, num_layers, tp)
                        model_cur_mem = model_cur_mem / pp
                        model_state_mem = model_state_mem / pp
                        
                        # LLM의 Activation 메모리는 L_pp에 독립적이므로 미리 계산 가능
                        act_mem_val = 0
                        if not is_vision:
                            pp_llm_layers = num_layers / pp
                            act_mem_val = calculate_act_peak(llm_act_mem_factors, pp_llm_layers, tp, 1, L_MAX_SEQ_LEN) * pp
                            # act_mem_val = act_mem_val / pp
                            # print(f"act_mem_val: {act_mem_val:.2f} model_mem {model_mem:.2f} for tp : {tp}")

                        # 기본 메모리 제약 확인 (종속적이지 않은 부분)
                        peak_mem = calculate_peak_mem(model_cur_mem, model_state_mem, act_mem_val)
                        if peak_mem <= GPU_MEMORY:
                            configs[g].append({
                                'tp': tp, 'pp': pp, 'dp': dp,
                                'model_mem': [model_cur_mem, model_state_mem],
                                'peak_mem' : peak_mem
                            })
    return configs


# --- 4. 동적 계획법 실행 (Main DP Algorithm) ---
def find_optimal_strategy_dp(TOTAL_GPUS, vision_model_mem_factors, vision_act_mem_factors, llm_model_mem_factors, llm_act_mem_factors, vision_num_layers, llm_num_layers, cal_vision_thr_dict, cal_llm_thr_dict, V_FLOPS, V_THR, L_FLOPS, L_THR, GLOBAL_BATCHSIZE, GPU_MEMORY, ALPHA, BETA, MICRO_BATCHSIZE, MAX_BATCH_SIZE, L_MAX_SEQ_LEN):
    """DP를 사용하여 최적의 병렬화 전략을 찾습니다."""
    print("Step 1: Pre-computing valid configurations for Vision and LLM...")
    print("======================== Vision Configs ========================")
    vision_configs = precompute_configs(TOTAL_GPUS, vision_model_mem_factors, llm_act_mem_factors, vision_num_layers, L_MAX_SEQ_LEN, GPU_MEMORY, is_vision=True, tp_choices=cal_vision_thr_dict.keys())
    print("======================== LLM Configs ========================")
    llm_configs = precompute_configs(TOTAL_GPUS, llm_model_mem_factors, llm_act_mem_factors, llm_num_layers, L_MAX_SEQ_LEN, GPU_MEMORY, is_vision=False, tp_choices=cal_llm_thr_dict.keys())
    
    # dp_table[k] = k개의 GPU로 달성 가능한 최소 병목 시간
    dp_table = [float('inf')] * (TOTAL_GPUS + 1)
    # config_table[k] = 최적 시간을 달성한 구성 정보
    config_table = [None] * (TOTAL_GPUS + 1)

    print("Step 2: Running main DP to find the optimal split...")
    for k in range(1, TOTAL_GPUS + 1):
        # k개의 GPU를 i개(vision)와 k-i개(llm)로 분할
        for i in range(1, k):
            v_gpus = i
            l_gpus = k - i

            # 해당 GPU 수에 유효한 구성이 없으면 건너뛰기
            if not vision_configs[v_gpus] or not llm_configs[l_gpus]:
                print(f"V GPUS : {v_gpus}, L GPUS : {l_gpus} passed")
                continue

            # 모든 Vision/LLM 구성 조합을 확인
            for v_conf in vision_configs[v_gpus]:
                for l_conf in llm_configs[l_gpus]:
                    # --- 여기서 종속적인 메모리 제약 최종 확인 ---
                    v_pp, l_pp = v_conf['pp'], l_conf['pp']
                    v_tp = v_conf['tp']
                    v_dp = v_conf['dp']
                    l_dp = l_conf['dp']
                    l_tp = l_conf['tp']
                    
                    # Vision의 activation 메모리는 L_pp에 종속적
                    pp_vision_layers = vision_num_layers / v_pp
                    v_act_mem = ((v_pp + l_pp) * calculate_act_peak(vision_act_mem_factors, pp_vision_layers, v_tp, MAX_BATCH_SIZE, 1024)) * (l_dp / v_dp)
                    v_model_cur_mem, v_model_state_mem = v_conf['model_mem']
                    peak_mem = calculate_peak_mem(v_model_cur_mem, v_model_state_mem, v_act_mem)
                    print(f"v_act_mem : {v_act_mem:.2f}, v_peak_mem : {peak_mem:.2f} for v_pp: {v_pp}, l_pp: {l_pp}, v_tp: {v_tp}, l_tp : {l_tp} v_dp: {v_dp}, l_dp: {l_dp}")
                    if peak_mem > GPU_MEMORY:
                        continue # Vision 메모리 제약 위반
                    
                    # LLM 메모리는 이미 사전 계산에서 확인 완료
                    
                    # --- 성능 계산 및 DP 테이블 업데이트 ---
                    total_pp = v_pp + l_pp
                    # print(L_FLOPS, L_THR, l_conf['tp'], l_pp, l_conf['dp'], BETA, NUM_MICROBATCHES, total_pp)
                    if GLOBAL_BATCHSIZE % (MICRO_BATCHSIZE * l_conf['dp']) != 0:
                        continue
                    NUM_MICROBATCHES = GLOBAL_BATCHSIZE // (MICRO_BATCHSIZE * l_conf['dp']) 
                    duration_v = calculate_duration(V_FLOPS, V_THR, v_conf['tp'], v_pp, v_conf['dp'], ALPHA, NUM_MICROBATCHES, total_pp)
                    duration_l = calculate_duration(L_FLOPS, L_THR, l_conf['tp'], l_pp, l_conf['dp'], BETA, NUM_MICROBATCHES, total_pp)
                    # print(f"[v_tp : {v_conf['tp']}, v_pp : {v_conf['pp']}, l_tp : {l_conf['tp']}, l_pp : {l_conf['pp']}] duration_v: {round(duration_v, 2)}, duration_l: {round(duration_l, 2)}, Peak Mem: {round(peak_mem, 2)}")

                    bottleneck = max(duration_v, duration_l)
                    
                    if bottleneck < dp_table[k]:
                        dp_table[k] = bottleneck
                        config_table[k] = {
                            'vision_gpus': v_gpus,
                            'llm_gpus': l_gpus,
                            'V_tp': v_conf['tp'], 'V_pp': v_pp, 'V_dp': v_conf['dp'],
                            'L_tp': l_conf['tp'], 'L_pp': l_pp, 'L_dp': l_conf['dp'],
                            'duration_V': duration_v,
                            'duration_L': duration_l,
                            'bottleneck_duration': bottleneck,
                            'v_peak_mem' : peak_mem,
                            'l_peak_mem' : l_conf['peak_mem']
                            
                        }

    return config_table[TOTAL_GPUS]

def parallel_optimization(image_batch_list, llm_input_seq_list, mem_dict, thr_dict, mllm_model_name, vision_model_name, vision_model_size, llm_model_name, llm_model_size):
    # Vision configs
    vision_config = vision_configs[vision_model_name][vision_model_size]
    ...
    # Model state & activation memory 
    vision_model_mem_factors, vision_act_mem_factors = mem_dict["vision"]
    llm_model_mem_factors, llm_act_mem_factors = mem_dict["llm"]

    # Vision & LLM FLOPs, Throughput
    vision_flops = [vision_module_flops(img_batch_size, in_channels, img_h, img_w, patch_dim, vision_num_layers, hs, intermediate_size, llm_hidden_size)/1e12 for img_batch_size in image_batch_list]
    mean_image_batch = np.mean(image_batch_list)
    vision_thr_prof = thr_dict["vision"]
    cal_vision_thr_dict = {}
    for tp_size, vision_thr in vision_thr_prof.items():
        cal_vision_thr_dict[tp_size] = vision_thr[0](mean_image_batch * micro_batch_size)
    ...
    llm_thr_prof = thr_dict["llm"]
    cal_llm_thr_dict = {}
    for tp_size, llm_thrs in llm_thr_prof.items():
        attn_thr_tp, linear_thr_tp = llm_thrs[0]
        attn_thr = attn_thr_tp([llm_seq_len for llm_seq_len in llm_input_seq_list]).mean()
        linear_thr = linear_thr_tp([mean_seq_len * micro_batch_size])[0]
        llm_thr = llm_attn_ratio * attn_thr + llm_linear_ratio * linear_thr
        cal_llm_thr_dict[tp_size] = llm_thr

    alpha_dict = {}
    beta_dict = {}
    for idx, (tp_size, vision_tp_thr) in enumerate(cal_vision_thr_dict.items()):
        if idx == 0:
            prev_vision_tp_thr = vision_tp_thr
            continue
        alpha_dict[tp_size] = vision_tp_thr / prev_vision_tp_thr
        prev_vision_tp_thr = vision_tp_thr
    for idx, (tp_size, llm_tp_thr) in enumerate(cal_llm_thr_dict.items()):
        if idx == 0:
            prev_llm_tp_thr = llm_tp_thr
            continue
        beta_dict[tp_size] = llm_tp_thr / prev_llm_tp_thr
        prev_llm_tp_thr = llm_tp_thr
    # --- 1. 상수 및 파라미터 정의 (Constants and Parameters) ---
    # 실제 모델과 하드웨어에 맞게 수정해야 합니다.
    buffer_ratio = 1
    TOTAL_GPUS = 64
    GPU_MEMORY = 80 * buffer_ratio

    # Vision 모델 관련
    V_FLOPS = sum(vision_flops)
    V_SEQ_LEN = 1024
    V_THR = cal_vision_thr_dict[1]
    ALPHA = alpha_dict

    # LLM 모델 관련
    L_FLOPS = sum(llm_flops)
    L_THR = cal_llm_thr_dict[1]
    BETA = beta_dict

    GLOBAL_BATCHSIZE = 256
    L_MAX_SEQ_LEN = np.max(llm_input_seq_list) * micro_batch_size
    MAX_BATCH_SIZE = np.max(image_batch_list) * micro_batch_size


    optimal_config = find_optimal_strategy_dp(TOTAL_GPUS, vision_model_mem_factors, vision_act_mem_factors, llm_model_mem_factors, llm_act_mem_factors, vision_num_layers, llm_num_layers, cal_vision_thr_dict, cal_llm_thr_dict, V_FLOPS, V_THR, L_FLOPS, L_THR, GLOBAL_BATCHSIZE, GPU_MEMORY, ALPHA, BETA, micro_batch_size, MAX_BATCH_SIZE, L_MAX_SEQ_LEN)

    print("\n" + "="*50)
    print("🏆 Optimal Strategy Found using Dynamic Programming 🏆")
    print("="*50)
    ...
    if optimal_config:
        ...
        print(f"No valid configuration found for {TOTAL_GPUS} GPUs.")
    return optimal_config

if __name__ == "__main__":
    result_path = "/giant-data/user/1113870/BDAI/paper_results/profiler_result"
    # Target Vision model and LLM
    ...
    # Data Discribution Analysis Results
    ...
    # 3D parallelism optimization
    optimal_configs = parallel_optimization(image_batch_list, llm_input_seq_list, mem_dict, thr_dict, mllm_model_name, vision_model_name, vision_model_size, llm_model_name, llm_model_size)
# python /giant-data/user/1113870/BDAI/dmllm_codes/dmllm_3d_opt.py