...
class InternVLInternVitModule(nn.Module):
    ...
    def forward(self,
        ...
        print(f"[Rank : {torch.distributed.get_rank()}] InternVLInternVitModule output shape : {output.shape}")
        return output
...
class VisionPipeStage(nn.Module):
    def __init__(self, vision_model, vision_stages, stage_id, device, model_dtype, is_internvl=False):
        ...
        # if not is_internvl and (stage_id == vision_stages[-1]):
...
class OursPipeStage(TransformerDecoder):
    def __init__(self, llm_model, stage_id, vision_stages, llm_stages, vision_module, device, config, llm_config, model_dtype):
        print(f"[Rank : {torch.distributed.get_rank()}] Initializing OursPipeStage with stage_id: {stage_id}, num_stages: {vision_stages + llm_stages}")
        model = llm_model
