...
class DataCollatorForSupervisedDataset(object):
    ...
    def __init__(self, tokenizer, model, dtype):
        model_copy = deepcopy(model)
        self.tokenizer = tokenizer
        self.config = model_copy.config
        self.get_2dPool = model_copy.get_2dPool
        self.add_token_per_grid = model_copy.add_token_per_grid
        self.num_patches_per_side = model_copy.get_vision_tower().num_patches_per_side
        self.image_newline = model_copy.model.image_newline
        self.image_size = model_copy.get_vision_tower().image_size if hasattr(model_copy.get_vision_tower(), "image_size") else None
        self.dtype = dtype
        ...
        for image_idx, image_feature in enumerate(image_features):
            if image_idx in video_idx_in_batch:  # video operations
                ...
                if "unpad" in mm_patch_merge_type:
                    # print(f"Rank : {torch.distributed.get_rank()} : image_feature : {image_feature}, image_newline : {self.image_newline[None]}, image_feature shape : {image_feature.shape}, image_newline shape : {self.image_newline[None].shape}")
    ...
    # def process_qwen_inputs(self, input_ids, attention_mask, labels, image_features):
    ...
    #     return new_input_embeds, attention_mask, new_labels
    ...
    #         fake_image_features = torch.zeros(concat_images.shape[0], 729, 3584, dtype=self.dtype)
    #         fake_image_features = self.process_image_features(fake_image_features, split_sizes, video_idx_in_batch, image_sizes)
    #         attention_mask = input_ids.ne(self.tokenizer.pad_token_id)
    #         _, new_attention_mask, new_labels = self.process_qwen_inputs(input_ids, attention_mask, labels, fake_image_features)
    #         batch["new_attention_mask"] = new_attention_mask
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            fake_image_features = self.process_image_features(fake_image_features, split_sizes, video_idx_in_batch, image_sizes)
            attention_mask = input_ids.ne(self.tokenizer.pad_token_id)
            _, new_position_ids, new_labels = self.process_qwen_inputs(input_ids, attention_mask, labels, fake_image_features)
